...
???+ "交錯路 (Alternating Path) 及增廣路 (Agumenting Path)"
    交錯路：序經過非匹配邊、匹配邊、...、非匹配邊、匹配邊、非匹配邊所形成的路徑。
    
    增廣路：從匹配點出發，經過交錯路，途中經過另一個為匹配點(起點除外)，該路徑稱為增廣路。把擴充路徑上的非匹配邊和匹配邊替換，就能使匹配數量 $+1$。
...
???+ "Berge's Theorem"
    ...
    此定理可延伸出，如果一個為匹配點 $v$ 找不到增廣路，那麼存在不包含 $v$ 的匹配 $M'$。

根據 Berge's Theorem，我們得到一個算法：枚舉集合 $X$ 未匹配的點 $s$，如果找到增廣路，則翻轉所有邊，否則就把 $s$ 移出匹配。找出增廣路的方式為，從 $X$ 集合的每個點 $s$ 開始 DFS，去拜訪集合 $Y$ 的每個和 $x$ 相連的點 $y$，如果 $y$ 是未匹配邊，則找到一條增廣路；如果 $y$ 是匹配邊，則從和 $y$ 匹配點 $x'$ 開始 DFS 尋找增廣路。

$X$ 集合個每個點都匹配一次，最多有 $V$ 個點，每次 DFS 的最多找到長度為 $E$ 的增廣路，整體時間複雜度為 $O(VE)$。
...
- 最小點覆蓋：如果匹配點有連接到未匹配點，那麼該點包含在最小點覆蓋中。
- 最大點獨立集：剛好跟最小點覆蓋互補。
